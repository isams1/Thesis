...
class RtDefaultSetView(view_base.ViewBase):
    def __init__(self, host, port, dpid, gateway, status=None):
        super(RtDefaultSetView, self).__init__()
        self.host = host
        self.port = port
        self.dpid = dpid
    ...
    def run(self):
        LOG.debug('Router Default Route Setting: Enter polling loop')
        if not self.status:
            # dump flows
    ...
    def _dump_status(self):
        ...
        rt_address = {}
        rt_address['gateway'] = self.gateway
